...
import {
  ...
  PanelLeftClose, PanelRightClose, ShieldCheck,
  ChevronDown
} from "lucide-react";
...
// --- MODIFICATION: Import the missing useWebSocket hook ---
...
const ValidationSidebar = memo(({
  ...
  const renderCategorizedList = () => {
    if (isDiscovering) return <div className="space-y-3">{Array.from({ length: 8 }).map((_, i) => <Skeleton key={i} className="h-7 w-full" />)}</div>;
    if (discoveryError) return <Alert variant="destructive"><XCircle className="h-4 w-4" /><AlertTitle>Error</AlertTitle><AlertDescription>Failed to load validations.</AlertDescription></Alert>;
    const testCategories = Object.entries(categorizedTests);
    if (testCategories.length === 0) return <p className="text-sm text-center text-gray-500">No validations available.</p>;
            ...
            <CollapsibleContent className="space-y-3 pl-2 pt-2">
              {tests.map((test) => (
                <div key={test.id} className="flex items-center space-x-3">
                  ...
                  />
                  <Label htmlFor={test.id} className="font-normal text-sm cursor-pointer">{test.description}</Label>
                </div>
              ))}
            </CollapsibleContent>
          </Collapsible>
        ...
        <div className="flex items-center h-16 border-b px-4 shrink-0">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Select tests to run</p>
                </div>
            </div>
        </div>
...
function ValidationRunner({ script, parameters, onParamChange }) {
  ...
  const { executionState, runValidationScript, resetExecution, categorizedTests, isDiscovering, discoveryError } = useValidationWorkflow(wsContext);
  ...
  const validateConfiguration = () => {
    ...
    if (!parameters.tests || parameters.tests.length === 0) return { isValid: false, reason: "At least one validation must be selected." };
  ...
  const handleRun = async (event) => {
    ...
    try { await runValidationScript(parameters); } catch (error) { toast.error(`Error starting script: ${error.message}`); }
  };

  const handleSave = async () => {
    if (!executionState.result) { toast.error("No results to save."); return; }
    try { await ValidationApiService.saveResults(executionState.result); toast.success("Validation Report saved successfully!"); } catch (error) { toast.error(`Error saving report: ${error.message}`); }
  };
    ...
    <div className="flex h-full w-full bg-muted/40 rounded-lg border overflow-hidden">
      <TooltipProvider delayDuration={150}>
        ...
        <main className="flex-1 flex flex-col min-w-0 relative">
            <Tooltip>
                <TooltipTrigger asChild><Button variant="outline" size="icon" className="absolute left-4 top-4 h-9 w-9 z-20" onClick={() => setIsSidebarCollapsed(v => !v)}>{isSidebarCollapsed ? <PanelRightClose className="h-4 w-4" /> : <PanelLeftClose className="h-4 w-4" />}</Button></TooltipTrigger>
            ...
            <div className="flex-1 flex flex-col">
                <header className="flex h-16 items-center gap-4 border-b bg-background px-4 md:pl-20 md:px-6 sticky top-0 z-10">
                    <Sheet>
                        ...
                        <SheetContent side="left" className="flex flex-col w-[280px] p-0">
                            <div className="flex-1 overflow-y-auto">
                                <ValidationSidebar categorizedTests={categorizedTests} isDiscovering={isDiscovering} discoveryError={discoveryError} selectedTests={parameters.tests || []} onSelectionChange={onParamChange} isCollapsed={false} />
                            </div>
                        </SheetContent>
                    </Sheet>
                    ...
                    <Button onClick={handleRun} disabled={isRunDisabled}><PlayCircle className="h-4 w-4 mr-2" />{executionState.isRunning ? "Running..." : "Run Validations"}</Button>
                </header>
                            ...
                            <TabsContent value="results" className="space-y-6">
                                ...
                                <>
                                    <div className="flex justify-between items-center p-4 bg-green-50 border border-green-200 rounded-lg">
                                        <div><h3 className="text-lg font-semibold text-green-800">Execution Completed</h3><p className="text-sm text-green-700">Validation results are available below.</p></div>
                                    ...
                                    <div className="text-center py-12 bg-gray-50 border-2 border-dashed rounded-lg"><Table2 className="h-12 w-12 text-gray-400 mx-auto mb-4" /><h3 className="text-lg font-medium">No Results Yet</h3><p className="text-gray-600">Run one or more validations to see results here.</p></div>
                                )}
                            </TabsContent>
                        </div>
                    </Tabs>
